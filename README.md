...
12. `매핑 결과 엑셀 저장` 버튼으로 결과 파일을 저장합니다.
...
빌드 결과는 `dist\BioFlowGBIF_v1.0.0\BioFlowGBIF.exe`처럼 생성됩니다. 같은 버전 폴더가 이미 있으면 덮어쓰지 않고 중단됩니다. 필요하면 `.\build_exe.ps1 -Version 1.0.1`처럼 직접 지정한 버전으로 빌드할 수도 있습니다.

빌드가 완료되면 실행 파일은 아래 경로에 생성됩니다.

```text
dist\BioFlowGBIF\BioFlowGBIF.exe
```

배포할 때는 `dist\BioFlowGBIF` 폴더 전체를 압축해서 전달하는 것을 권장합니다. 실행 파일과 필요한 라이브러리가 같은 폴더에 함께 들어 있기 때문입니다.

## 배포 전 확인 사항

- `dist\BioFlowGBIF\BioFlowGBIF.exe`를 실행해 프로그램이 열리는지 확인합니다.
