...
6. 매핑 전에 좌표만 먼저 확인하려면 `원본 좌표 확인` 버튼을 누르고 위도/경도 컬럼을 선택합니다.
7. 오른쪽 `GBIF 필드 매핑` 영역에서 자동 매핑된 컬럼을 확인합니다.
8. 필요한 경우 잘못 매핑된 항목을 직접 선택합니다.
9. 기본 컬럼 외에 더 필요한 GBIF 컬럼은 `컬럼 추가` 버튼으로 추가합니다.
10. `결과 확인` 버튼을 누릅니다.
11. 결과 테이블에서 값을 확인하고 필요하면 직접 수정합니다.
12. 아래 `빈칸 점검` 영역에서 빈칸 컬럼과 위치를 확인합니다.
13. `좌표 확인` 버튼으로 `decimalLatitude`, `decimalLongitude` 값을 지도에서 확인합니다.
...
`선택 행을 헤더로 적용`한 뒤 `원본 좌표 확인` 버튼을 누르면 원본 데이터의 위도/경도 컬럼을 직접 선택해 매핑 전에 좌표 위치를 확인할 수 있습니다.
